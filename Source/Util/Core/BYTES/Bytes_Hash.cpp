...
namespace Dv::Core
{
		size_t Calc_OneAtATime_LookupHash(const void *pBytes, size_t InNumBytes)
		{
			...
			{
				auto pCurrByte = static_cast<const uint8_t *>(AddPtr(pBytes, curr_byte_index));
				result += *pCurrByte;
				result += (result << 10);
				result ^= (result >> 6);
			}
		...
		{
			auto pResultByte = static_cast<const void *>(AddPtr(pTypedBytes, InByteIndex));
		...
		size_t Calc_Jenkins(const void *pBytes, size_t InNumBytes, size_t InInitValue)
		{
			...
			while (len >= 12)
			{
				a += (*AddPtr(pTypedBytes, 0) + (Jenkins_GetShiftedInputWord(pTypedBytes, 1, 8) + Jenkins_GetShiftedInputWord(pTypedBytes, 2, 16) + Jenkins_GetShiftedInputWord(pTypedBytes, 3, 24)));
				b += (*AddPtr(pTypedBytes, 4) + (Jenkins_GetShiftedInputWord(pTypedBytes, 5, 8) + Jenkins_GetShiftedInputWord(pTypedBytes, 6, 16) + Jenkins_GetShiftedInputWord(pTypedBytes, 7, 24)));
				c += (*AddPtr(pTypedBytes, 8) + (Jenkins_GetShiftedInputWord(pTypedBytes, 9, 8) + Jenkins_GetShiftedInputWord(pTypedBytes, 10, 16) + Jenkins_GetShiftedInputWord(pTypedBytes, 11, 24)));
