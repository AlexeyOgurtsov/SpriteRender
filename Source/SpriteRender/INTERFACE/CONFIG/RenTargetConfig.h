...
{
	ID3D11RenderTargetView* const pView = InConfig.pRenderTargetView;
	BOOST_ASSERT_MSG(pView, "SpriteRender config is invalid: render target view pointer must be valid");
	D3D11_RENDER_TARGET_VIEW_DESC desc;
	pView->GetDesc(&desc);
	BOOST_ASSERT_MSG(desc.ViewDimension == D3D11_RTV_DIMENSION_TEXTURE2D, "SpriteRender config is invalid: render target dimension must be Texture2D");
	BOOST_ASSERT_MSG(desc.Format == DXGI_FORMAT_R8G8B8A8_UNORM, "SpriteRender config is invalid: render target format is invalid");
}

inline void AssertRenderTargetConfig_DepthStencilViewValid(const SRenTargetConfig& InConfig)
{
	ID3D11DepthStencilView* const pView = InConfig.pDepthStencilView;
	BOOST_ASSERT_MSG(pView, "SpriteRender config is invalid: depth-stencil view pointer must be valid");
	D3D11_DEPTH_STENCIL_VIEW_DESC desc;
	pView->GetDesc(&desc);
	BOOST_ASSERT_MSG(desc.ViewDimension == D3D11_DSV_DIMENSION_TEXTURE2D, "SpriteRender config is invalid: depth stencil view dimension must be Texture2D");
	BOOST_ASSERT_MSG( ! (D3D11_DSV_READ_ONLY_DEPTH & desc.Flags ), "SpriteRender config: depth stencil view must be writable");
}
...
{
	BOOST_ASSERT_MSG(InConfig.RTWidth > 0, "SpriteRender config is invalid: Render target width must be greater than zero");
	BOOST_ASSERT_MSG(InConfig.RTHeight > 0, "SpriteRender config is invalid: Render target height must be greater than zero");
	AssertRenderTargetConfig_RTViewValid(InConfig);
	AssertRenderTargetConfig_DepthStencilViewValid(InConfig);
	BOOST_ASSERT_MSG(InConfig.ZFar >= 0 && InConfig.ZFar <= 1.0F, "SpriteRender config is invalid: ZFar must be in range [0.0F; 1.0F]");
